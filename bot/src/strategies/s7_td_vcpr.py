"""Strategy 6 — Virgin CPR Scanner (Multi-Day, All Pairs).

Mirrors Pine Script Module D logic exactly:
  - Pivot  = (PDH + PDL + PDC) / 3
  - BC     = (PDH + PDL) / 2
  - TC     = 2 × Pivot − BC
  - Band   = [min(TC, BC), max(TC, BC)]
  - Virgin = no 1H candle since that day has touched the band
             (condition: candle_high >= band_bot AND candle_low <= band_top)
  - Wide   = band_width > ATR(14) × WIDE_FACTOR  →  strong magnet zone

Data source: TradingView via tvDatafeed
  - Daily bars   → Interval.in_daily       (LOOKBACK_DAYS + ATR_PERIOD + 2 bars)
  - Intraday bars → Interval.in_1_hour     (LOOKBACK_DAYS × 24 bars, enough history)

This strategy does NOT produce a directional Signal on its own.
It is called by run_signals.py → vcpr_scan_all_pairs(), which:
  1. Runs check() on every pair to get the VCprReport
  2. Sends a consolidated Telegram summary once per session
  3. Appends results to a dedicated state file so repeats are suppressed

Returns:
  VCprReport  — always (even if empty), never None.
  The caller decides whether to alert based on .has_virgins.
"""
from __future__ import annotations

from dataclasses import dataclass, field
from typing import Optional
import logging

import pandas as pd
from tvDatafeed import TvDatafeed, Interval

log = logging.getLogger(__name__)

NAME  = "Virgin CPR"
SID   = "S6"

# ─── tunables ───────────────────────────────────────────────────────────────
LOOKBACK_DAYS  = 30      # how many past daily bars to inspect
WIDE_FACTOR    = 1.0     # band_width > WIDE_FACTOR × ATR(14)  →  wide CPR
ATR_PERIOD     = 14

# ─── shared TvDatafeed instance (re-used across all pairs) ──────────────────
_tv = TvDatafeed()


# ─── data class ─────────────────────────────────────────────────────────────
@dataclass
class VirginBand:
    date:      str     # "YYYY-MM-DD"  — the day this CPR was active
    pivot:     float
    bc:        float
    tc:        float
    band_top:  float   # max(tc, bc)
    band_bot:  float   # min(tc, bc)
    width:     float   # band_top − band_bot
    atr14:     float
    is_wide:   bool
    is_virgin: bool = True

    @property
    def width_pct(self) -> float:
        return (self.width / self.pivot * 100) if self.pivot else 0.0

    @property
    def tag(self) -> str:
        return "🔴 WIDE" if self.is_wide else "🟣 Normal"

    def one_liner(self) -> str:
        return (
            f"{self.tag} | {self.date} | "
            f"Pivot {self.pivot:.5f} | "
            f"Band [{self.band_bot:.5f}–{self.band_top:.5f}] | "
            f"W={self.width_pct:.2f}%"
            + (" ⚠️ WIDE" if self.is_wide else "")
        )


@dataclass
class VCprReport:
    symbol:  str
    bands:   list[VirginBand] = field(default_factory=list)   # ALL virgin bands found

    @property
    def has_virgins(self) -> bool:
        return bool(self.bands)

    @property
    def wide_count(self) -> int:
        return sum(1 for b in self.bands if b.is_wide)

    @property
    def normal_count(self) -> int:
        return sum(1 for b in self.bands if not b.is_wide)

    def summary_lines(self) -> list[str]:
        """Return formatted lines for Telegram message (sorted newest first)."""
        lines = []
        for b in sorted(self.bands, key=lambda x: x.date, reverse=True):
            lines.append(f"  • {b.one_liner()}")
        return lines


# ─── TvDatafeed helpers ──────────────────────────────────────────────────────
def _tv_fetch(
    symbol: str,
    exchange: str,
    interval: Interval,
    n_bars: int,
) -> pd.DataFrame:
    """
    Fetch OHLCV from TradingView and return a clean DataFrame with:
      - DatetimeIndex (UTC, tz-naive)
      - columns: open, high, low, close, volume  (all lower-case)
    """
    raw = _tv.get_hist(symbol=symbol, exchange=exchange, interval=interval, n_bars=n_bars)
    if raw is None or raw.empty:
        raise ValueError(f"tvDatafeed returned no data for {exchange}:{symbol} @ {interval}")

    raw = raw.reset_index()

    # tvDatafeed names the datetime column 'datetime' after reset_index
    if "datetime" in raw.columns:
        raw["datetime"] = pd.to_datetime(raw["datetime"], utc=True).dt.tz_localize(None)
        raw = raw.set_index("datetime")
    elif raw.index.name == "datetime" or hasattr(raw.index, "strftime"):
        raw.index = pd.to_datetime(raw.index, utc=True).tz_localize(None)

    raw.columns = [c.lower() for c in raw.columns]
    raw = raw[["open", "high", "low", "close", "volume"]].dropna()
    raw.sort_index(inplace=True)
    return raw

def cal_cpr(day_data):
    df = day_data.copy()

    for i in range(1, len(df)):
        prev_high = df.loc[i - 1, 'high']
        prev_low = df.loc[i - 1, 'low']
        prev_close = df.loc[i - 1, 'close']

        pp = (prev_high + prev_low + prev_close) / 3
        bcpr = (prev_high + prev_low) / 2
        tcpr = (pp - bcpr) + pp

        # ensure BCPR < TCPR
        bcpr, tcpr = min(bcpr, tcpr), max(bcpr, tcpr)

        df.loc[i, 'PP'] = pp
        df.loc[i, 'BCPR'] = bcpr
        df.loc[i, 'TCPR'] = tcpr

    return df
def find_virgin_cpr(df):
    """
    Find all Virgin CPRs.
    Returns:
        date
        bcpr
        tcpr
        width
    """

    virgin_cprs = []

    for i in range(len(df) - 1):
        cpr_low = df.loc[i, 'BCPR']
        cpr_high = df.loc[i, 'TCPR']
        cpr_date = df.loc[i, 'datetime']

        touched = False

        # check all future candles
        for j in range(i , len(df)):
            future_high = df.loc[j, 'high']
            future_low = df.loc[j, 'low']

            # if future candle touches CPR zone
            if future_high >= cpr_low and future_low <= cpr_high:
                touched = True
                break

        if not touched:
            virgin_cprs.append({
                "date": cpr_date,
                "bcpr": cpr_low,
                "tcpr": cpr_high,
                "width": abs(cpr_high - cpr_low)
            })

    return pd.DataFrame(virgin_cprs)
def fetch_data(
    symbol: str,
    exchange: str = "NSE",
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Pull the two DataFrames needed by check():

      daily  — Interval.in_daily  with LOOKBACK_DAYS + ATR_PERIOD + 5 bars
                (extra buffer so ATR is warm by the time we use it)

      df_1h  — Interval.in_1_hour with LOOKBACK_DAYS × 25 bars
                (≈ 25 trading hours per calendar day covers weekends / gaps)

    Returns (df_1h, daily).
    Raises ValueError / any tvDatafeed exception if data cannot be fetched.
    """
    last_30 = _tv.get_hist(
        symbol=symbol,
        exchange=exchange,
        interval=Interval.in_daily,
        n_bars=15
    )

    last_30.reset_index(inplace=True)
    last_30.rename(columns={'index': 'datetime'}, inplace=True)

    cpr_data = cal_cpr(last_30)

    vcpr_df = find_virgin_cpr(cpr_data)
    vcpr_df = vcpr_df.iloc[1:].reset_index(drop=True)
    vcpr_tuples = list(vcpr_df.itertuples(index=False, name=None))
    print(vcpr_tuples)
    return vcpr_tuples


# ─── internal helpers ────────────────────────────────────────────────────────
def _calc_atr(daily_df: pd.DataFrame, period: int = 14) -> pd.Series:
    """ATR via exponential smoothing of True Range — same as Pine ta.atr()."""
    df = daily_df.copy()
    prev = df["close"].shift(1)
    df["tr"] = pd.concat([
        df["high"] - df["low"],
        (df["high"] - prev).abs(),
        (df["low"]  - prev).abs(),
    ], axis=1).max(axis=1)
    return df["tr"].ewm(span=period, adjust=False).mean()


def _build_bands(daily_df: pd.DataFrame, lookback: int) -> list[VirginBand]:
    """
    Compute CPR bands for the last `lookback` completed days.
    Each band uses the PREVIOUS day's H/L/C (mirrors Pine: high[1], low[1], close[1]).
    """
    df = daily_df.reset_index(drop=True)
    atr_s = _calc_atr(df, ATR_PERIOD)
    bands: list[VirginBand] = []

    # We need row i-1 for H/L/C; row i just supplies the date label
    start = max(1, len(df) - lookback)
    for i in range(start, len(df)):
        prev = df.iloc[i - 1]
        ph, pl, pc = float(prev["high"]), float(prev["low"]), float(prev["close"])
        pivot     = (ph + pl + pc) / 3.0
        bc        = (ph + pl) / 2.0
        tc        = 2.0 * pivot - bc
        band_top  = max(tc, bc)
        band_bot  = min(tc, bc)
        width     = band_top - band_bot
        atr14     = float(atr_s.iloc[i - 1]) if i - 1 < len(atr_s) else 0.0
        is_wide   = (atr14 > 0) and (width > atr14 * WIDE_FACTOR)

        # Date label: use the DatetimeIndex (tvDatafeed always gives one after _tv_fetch)
        if hasattr(daily_df.index, "strftime"):
            # daily_df still has its original index; map position i back to it
            date_str = daily_df.index[i].strftime("%Y-%m-%d")
        elif "date" in df.columns:
            date_str = str(df["date"].iloc[i])[:10]
        else:
            date_str = f"day-{i}"

        bands.append(VirginBand(
            date=date_str, pivot=pivot, bc=bc, tc=tc,
            band_top=band_top, band_bot=band_bot,
            width=width, atr14=atr14, is_wide=is_wide,
        ))
    return bands


def _mark_touched(bands: list[VirginBand], df_1h: pd.DataFrame) -> list[VirginBand]:
    """
    For each band, scan all 1H bars AFTER that date.
    Touch condition (Pine Module D):  high >= band_bot  AND  low <= band_top
    """
    for band in bands:
        if hasattr(df_1h.index, "strftime"):
            future = df_1h[df_1h.index.strftime("%Y-%m-%d") > band.date]
        elif "date" in df_1h.columns:
            future = df_1h[df_1h["date"].astype(str).str[:10] > band.date]
        else:
            future = df_1h   # fallback: can't filter, assume no touch

        if future.empty:
            continue   # no bars after that date → still virgin

        touched = (
            (future["high"] >= band.band_bot) &
            (future["low"]  <= band.band_top)
        ).any()
        band.is_virgin = not touched
    return bands


# ─── public API — called by run_signals.py ───────────────────────────────────
def check(
    symbol: str,
    exchange: str = "NSE",
    *,
    df_1h:  Optional[pd.DataFrame] = None,
    daily:  Optional[pd.DataFrame] = None,
) -> Optional[VCprReport]:
    """
    Fetch data from TradingView (if not supplied) and return a VCprReport.

    Parameters
    ----------
    symbol   : TradingView ticker, e.g. "NIFTY", "BANKNIFTY", "EURUSD"
    exchange : TradingView exchange, e.g. "NSE", "FX", "BINANCE"
    df_1h    : pre-fetched 1H DataFrame (optional — skips network call)
    daily    : pre-fetched daily DataFrame (optional — skips network call)

    Returns None only if data is insufficient (caller should log and skip).
    """
    report = fetch_data(symbol, exchange)
    # ── 1. Fetch data from TradingView if not pre-supplied ──────────────────


    # ── 2. Guard: need enough daily bars ────────────────────────────────────


    # ── 5. Build and return report ───────────────────────────────────────────


    return report